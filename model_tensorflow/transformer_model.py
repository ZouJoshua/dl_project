...
class Transformer(BaseModel):
    ...
    def __init__(self, config, vocab_size, word_vectors):
        super(Transformer, self).__init__(config=config, vocab_size=vocab_size, word_vectors=word_vectors)
    ...
    def transformer_layer(self):
        ...
        with tf.name_scope("transformer-layer"):
            for i in range(self.config.num_blocks):
                with tf.name_scope("transformer-{}".format(i + 1)):
                    ...
                    with tf.name_scope("feed_forward"):
                        # 维度[batch_size, sequence_length, embedding_dim]
                        self.embedded_representation = self._feed_forward(multihead_atten,
                                                                     [self.config.filters,
                                                                      self.config.embedding_dim])

            self.transformer_outputs = tf.reshape(self.embedded_representation,
                                 [-1, self.config.sequence_length * self.config.embedding_dim])
    ...
    def full_connection_layer(self):
        ...
        with tf.name_scope("dropout"):
            h_drop = tf.nn.dropout(self.transformer_outputs, self.keep_prob)


        # 全连接层的输出
        output_size = self.transformer_outputs.get_shape()[-1].value
        with tf.name_scope("fully_connection_layer"):
            output_w = tf.get_variable(
                "output_w",
                shape=[output_size, self.config.num_labels],
                initializer=tf.contrib.layers.xavier_initializer())
    ...
    def _position_embedding(self, batch_size):
        """
        生成位置向量
        :param batch_size: 根据训练\评估\测试动态调整
        :return:
        """

        # 生成位置的索引，并扩张到batch中所有的样本上
        position_index = tf.tile(tf.expand_dims(tf.range(self.config.sequence_length), 0), [batch_size, 1])

        # 根据正弦和余弦函数来获得每个位置上的embedding的第一部分
        position_embedding = np.array([[pos / np.power(10000, (i - i % 2) / self.config.embedding_dim)
        ...
        # 将position_embedding转换成tensor的格式
        position_embedding = tf.cast(position_embedding, dtype=tf.float32)

        # 得到三维的矩阵[batch_size, sequence_length, embedding_dim]
    ...
    def _multihead_attention(self, inputs, queries, keys, num_units=None):
        ...
        # 将多头Attention计算的得到的输出重组成最初的维度[batch_size, sequence_length, embedding_dim]
    ...
    def _layer_normalization(self, inputs):
        ...
        inputs_shape = inputs.get_shape()  # [batch_size, sequence_length, embedding_dim]
