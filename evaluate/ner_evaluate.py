...
"""
@Author  : Joshua
@Time    : 5/11/20 11:23 AM
@File    : ner_evaluate.py
@Desc    : 
...
import json
import time
from nlp_tasks.information_extraction.ie_plots import plot_classification_report, heatmap
from nlp_tasks.information_extraction.ie_preprocess import remove_bio_from_label_name, get_parsed_conll_output
...
def assess_model(y_pred, y_true, labels, target_names, labels_with_o, target_names_with_o, dataset_type, stats_graph_folder, epoch_number, parameters,
    ...
    heatmap(confusion_matrix, title, xlabel, ylabel, xticklabels, yticklabels, figure_width=40, figure_height=20, correct_orientation=True, fmt="%d",
...
def evaluate_model(results, dataset, y_pred_all, y_true_all, stats_graph_folder, epoch_number, epoch_start_time, output_filepaths, parameters, verbose=False):
    ...
    for dataset_type in ['train', 'valid', 'test']:
        ...
        conll_parsed_output = get_parsed_conll_output(conll_output_filepath)
        ...
        if parameters['main_evaluation_mode'] == 'conll':
            ...
            plot_classification_report(results['epoch'][epoch_number][0][dataset_type]['conll'],
