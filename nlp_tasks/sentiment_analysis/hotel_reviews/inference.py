...
class SentimentAnalysis:
    ...
    def load_model(self, model, dir_path="../output"):
        ...
        # 这时我们不载入模型原本用于训练Next Sentence的pooler
        # 而是重新初始化了一个
...
if __name__ == '__main__':
    import time
    s = time.time()
    model = SentimentAnalysis(max_seq_len=300, batch_size=1)
    s1 = time.time()
    print("模型加载耗时: {}s".format(s1-s))
    ...
    # ]
    test_list = ["这个酒店不是很干净,我不太喜欢,交通便利但是有点吵",
                 "这个酒店不是很干净,我太不喜欢了,交通便利但是有点吵",
                 "这个酒店不是很干净,我太不喜欢了,有点吵"]
    model(test_list)
    s2 = time.time()
    print("模型预测10条样本耗时:{}s".format(s2-s1))