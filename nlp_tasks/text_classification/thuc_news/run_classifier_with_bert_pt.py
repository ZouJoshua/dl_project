...
class Config(object):
    ...
    def __init__(self, config_file, section=None):
        ...
        self.init_checkpoint = config.get("init_checkpoint")
        self.word2idx_path = config.get("word2idx_path")
...
class Trainer:
    ...
    def __init__(self, config, logger=None,
        ...
        self.train_corpus_path = os.path.join(config.data_path, "thuc_news.eval.txt")
        self.test_corpus_path = os.path.join(config.data_path, "thuc_news.test.txt")
        ...
        self.init_optimizer(lr=self.config.learning_rate)

        if self.config.ckpt_model_path:
            self.ckpt_model_path = self.config.ckpt_model_path
        else:
            self.ckpt_model_path = os.path.join(self.config.output_path, "model")

        if not os.path.exists(self.ckpt_model_path):
    ...
    def load_model(self, model, dir_path="../output", load_bert=False):
        ...
        self.log.info("{} loaded!".format(checkpoint_dir))
    ...
    def find_most_recent_state_dict(self, dir_path):
        ...
        return os.path.join(dir_path, dic_lis[-1])
    ...
    def save_state_dict(self, model, epoch, state_dict_dir="../output", file_path="bert.model"):
        ...
        save_path = os.path.join(state_dict_dir, file_path + ".epoch.{}".format(str(epoch)))
        model.to("cpu")
        torch.save({"model_state_dict": model.state_dict()}, save_path)
        self.log.info("{} saved!".format(save_path))
        model.to(self.device)
