...
"""
@Author  : Joshua
@Time    : 2/26/20 8:02 PM
@File    : words_mining.py
@Desc    : 新词挖掘
...
data_dir = os.path.join(DATA_PATH, "common")
pos_dict_file = os.path.join(data_dir, "pos_dict", "dict.txt.big")
stopwords_file = os.path.join(data_dir, "stopwords", "stopwords_zh.txt")
...
def loda_word_dict(stopwords_file, pos_dict_file):
    ...
    *结巴自带词典词性标注准确度一般, 可根据需要修改
    ...
    if os.path.exists(pos_dict_file):
        with open(pos_dict_file, "r", encoding="utf8") as fr:
            word_dict = {word.strip().split(" ")[0]: word.strip().split(" ")[2] for word in fr}

    if os.path.exists(stopwords_file):
        with open(stopwords_file, "r", encoding="utf8") as fr:
