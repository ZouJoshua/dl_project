...
"""
@Author  : Joshua
@Time    : 2/26/20 2:23 PM
@File    : train.py
@Desc    : Trains a seq2seq model using Sequence-to-sequence RNNS and Beyond
...
import nlp_tasks.textsum.batch_reader as batch_reader
import model_tensorflow.textsum_model.dataset_process as data
from model_tensorflow.textsum_model import seq2seq_attention_decode
from model_tensorflow.textsum_model import seq2seq_attention_model
...
tf.flags.DEFINE_string('article_key', 'article',
                           'tf.Example feature key for article.')
tf.flags.DEFINE_string('abstract_key', 'headline',
                           'tf.Example feature key for abstract.')
tf.flags.DEFINE_string('log_root', '', 'Directory for model root.')
tf.flags.DEFINE_string('train_dir', '', 'Directory for train.')
tf.flags.DEFINE_string('eval_dir', '', 'Directory for eval.')
tf.flags.DEFINE_string('decode_dir', '', 'Directory for decode summaries.')
tf.flags.DEFINE_string('mode', 'train', 'train/eval/decode mode')
tf.flags.DEFINE_integer('max_run_steps', 10000000,
                            'Maximum number of run steps.')
tf.flags.DEFINE_integer('max_article_sentences', 2,
                            'Max number of first sentences to use from the '
                            'article')
tf.flags.DEFINE_integer('max_abstract_sentences', 100,
                            'Max number of first sentences to use from the '
                            'abstract')
tf.flags.DEFINE_integer('beam_size', 4,
...
tf.flags.DEFINE_bool('use_bucketing', False,
                         'Whether bucket articles of similar length.')
tf.flags.DEFINE_bool('truncate_input', False,
